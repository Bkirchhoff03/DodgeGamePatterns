#include "C:\Users\nebk2\Unity\DodgeGamePatterns\Library\PackageCache\com.unity.render-pipelines.universal@ba18377e2205\ShaderLibrary\Lighting.hlsl"
#include "C:\Users\nebk2\Unity\DodgeGamePatterns\Library\PackageCache\com.unity.render-pipelines.universal@ba18377e2205\ShaderLibrary\ShaderVariablesFunctions.hlsl"
float LightingSpecular_float(float3 L, float3 N, float3 V, float smoothness)
{
...
void MainLighting_float(float3 normalWS, float3 positionWS, float3 viewWS, float smoothness, out float specular)
{
    ...
    specular = LightingSpecular_float(mainLight.direction, normalWS, viewWS, smoothness);
    #endif
}
...
void AdditionalLighting_float(float3 normalWS, float3 positionWS, float3 viewWS, float smoothness, float hardness, out float3 specular)
{
    ...
    for (int i = 0; i < pixelLightCount; ++i)
    {
        ...
        float specular_soft = LightingSpecular_float(light.direction, normalWS, viewWS, smoothness);
